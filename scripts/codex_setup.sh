...
# cudf and cuml are installed separately to allow more flexible GPU setup.
set -e
...
if ! conda info --envs | grep -q '^rapids-25.04'; then
  conda create -y -n rapids-25.04 -c rapidsai -c conda-forge -c nvidia \
    ...
    'pytorch=*=*cuda*' tensorflow dash networkx \
    nx-cugraph=25.04 jupyterlab graphistry xarray-spatial
fi
...
# Install requirements without GPU-related libraries
...
# Install cudf and cuml separately
