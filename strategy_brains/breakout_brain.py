...
from common.constants import OrderType, OrderSide, TimeInForce, SignalConfidence, SignalType
from common.utils import calculate_risk_reward, normalize_price_series
from common.exceptions import StrategyError, InvalidDataError

from feature_service.features.technical import calculate_atr
from feature_service.features.market_structure import identify_swing_points, detect_consolidation
...
from strategy_brains.base_brain import StrategyBrain, StrategyConfig, SignalEvent
...
class BreakoutBrain(StrategyBrain):
    ...
    async def analyze(self, market_data: pd.DataFrame, context: Dict[str, Any] = None) -> List[SignalEvent]:
        ...
        try:
            ...
            if self.config.higher_timeframe_confirmation and context and 'higher_timeframe_data' in context:
                breakout_signals = self._apply_higher_timeframe_confirmation(
                    breakout_signals, 
                    context['higher_timeframe_data']
                )
    ...
    def _identify_support_resistance(self, data: pd.DataFrame) -> Dict[str, List[float]]:
        ...
        sr_zones = identify_support_resistance_zones(
            data,
            zone_width_pct=self.config.sr_zone_width_percentage,
            min_touches=2,
            swing_strength=0.6
        )
    ...
    def _detect_chart_patterns(self, data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        rectangles = detect_rectangle_pattern(
            data,
            tolerance=0.02,
            min_touches=2
        )
        ...
        triangles = detect_triangle_pattern(
            data,
            min_points=5
        )
            ...
            if region['end_idx'] >= data.index[-self.config.lookback_periods]:
                ...
                if latest_close > region['high'] * (1 + self.config.price_confirmation_percentage / 100):
                    ...
                    if latest_volume > region['avg_volume'] * self.config.volume_confirmation_threshold:
                        ...
                        vol_expansion = volatility_expansion_indicator(
                            data, 
                            window1=5, 
                            window2=20
                        ).iloc[-1]
                        ...
                        if vol_expansion > self.config.volatility_expansion_threshold:
                            breakout_signals.append({
                                ...
                                'confidence': self._calculate_confidence(
                                    'consolidation', 
                                    latest_volume / region['avg_volume'],
                                    vol_expansion
                                ),
            ...
            if 'end_idx' in pattern and pattern['end_idx'] >= data.index[-self.config.lookback_periods]:
                ...
                if pattern['type'] == 'rectangle':
                    ...
                    if 'upper_trendline' in pattern:
                        current_upper = self._calculate_trendline_value(
                            pattern['upper_trendline'], 
                            pattern['start_idx'], 
                            data.index[-1]
                        )
                    
                    if 'lower_trendline' in pattern:
                        current_lower = self._calculate_trendline_value(
                            pattern['lower_trendline'], 
                            pattern['start_idx'], 
                            data.index[-1]
                        )
    ...
    def _apply_higher_timeframe_confirmation(
        self, 
        ...
        for signal in signals:
            ...
            for tf, tf_data in sorted(higher_tf_data.items()):
                ...
                if signal['direction'] == 'up':
                    ...
                    tf_trend_up = (
                        recent_tf_data['close'].iloc[-1] > ema20.iloc[-1] and
                    ...
                    if tf_trend_up:
                        ...
                        signal['confidence'] = min(
                            ...
                            signal['confidence'] + 1  # Increase confidence by one level
                        )
                        break
    ...
    def _count_previous_tests(self, data: pd.DataFrame, level: float, level_type: str) -> int:
        ...
        if level_type == 'resistance':
            ...
            for i in range(1, len(recent_data)):
                if (recent_data['high'].iloc[i] > level - price_buffer and 
                    recent_data['high'].iloc[i] < level + price_buffer):
                    count += 1
        else:  # support
            # Count number of times price approached support from above
            for i in range(1, len(recent_data)):
                if (recent_data['low'].iloc[i] < level + price_buffer and 
                    recent_data['low'].iloc[i] > level - price_buffer):
                    count += 1
    ...
    async def on_signal_result(self, signal: SignalEvent, result: Dict[str, Any]) -> None:
        ...
        if pattern_type not in self.breakout_success_metrics[asset]:
            self.breakout_success_metrics[asset][pattern_type] = {
                'wins': 0,
                'total': 0,
                'avg_profit': []
            }
        ...
        avg_profit = np.mean(self.breakout_success_metrics[asset][pattern_type]['avg_profit']) if self.breakout_success_metrics[asset][pattern_type]['avg_profit'] else 0
    ...
    def _adapt_parameters_based_on_performance(self, asset: str, pattern_type: str, success_rate: float, avg_profit: float) -> None:
        ...
        if success_rate < 0.4:  # Poor performance
            ...
            self.config.asset_specific_config[asset]['volume_confirmation_threshold'] = min(
                2.0, 
                self.config.volume_confirmation_threshold * 1.1
            )
            
            # Increase volatility requirements
            self.config.asset_specific_config[asset]['volatility_expansion_threshold'] = min(
                2.5, 
                self.config.volatility_expansion_threshold * 1.1
            )
            
            # Increase false breakout protection
            self.config.asset_specific_config[asset]['false_breakout_atr_multiplier'] = min(
                1.0, 
                self.config.false_breakout_atr_multiplier * 1.1
            )
            
            # Require higher minimum confidence
            self.config.asset_specific_config[asset]['min_signal_confidence'] = max(
                self.config.min_signal_confidence,
                SignalConfidence.MEDIUM
            )
            
        elif success_rate > 0.7:  # Excellent performance
            # Slightly relax parameters to take more trades
            self.config.asset_specific_config[asset]['volume_confirmation_threshold'] = max(
                1.3, 
                self.config.volume_confirmation_threshold * 0.95
            )
            
            # Allow slightly less volatility expansion
            self.config.asset_specific_config[asset]['volatility_expansion_threshold'] = max(
                1.5, 
                self.config.volatility_expansion_threshold * 0.95
            )
        ...
        if 'consolidation' in pattern_type:
            if success_rate < 0.4:
                ...
                self.config.asset_specific_config[asset]['min_consolidation_periods'] = min(
                    10, 
                    self.config.min_consolidation_periods + 1
                )
            elif success_rate > 0.7:
                # Allow shorter consolidation periods
                self.config.asset_specific_config[asset]['min_consolidation_periods'] = max(
                    3, 
                    self.config.min_consolidation_periods - 1
                )
