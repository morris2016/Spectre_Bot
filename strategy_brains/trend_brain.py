...
from common.utils import calculate_atr, zigzag_identification, is_higher_high, is_lower_low
from common.constants import (
    TREND_FOLLOWING_CONFIG, DIRECTIONAL_BIAS_THRESHOLD, FILTER_STRENGTH_LEVELS,
    REGIME_TYPES
)
from common.exceptions import StrategyError, InsufficientDataError, SignalGenerationError

from feature_service.features.technical import (
    calculate_moving_average, calculate_macd, calculate_adx, 
    calculate_supertrend, calculate_donchian_channel
)
from feature_service.features.volatility import calculate_keltner_channel
from feature_service.features.market_structure import identify_market_structure, analyze_swing_points
...
class TrendBrain(BaseBrain):
    ...
    async def analyze(self, data: Dict[str, pd.DataFrame], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Trend analysis complete. Direction: {overall_trend['trend_direction']}, Strength: {overall_trend['trend_strength']:.2f}")
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> None:
        ...
        self.current_swing_points = [
            {"type": "high", "price": price, "index": idx} 
            for idx, price in swing_points['swing_highs'].dropna().iloc[-5:].items()
        ] + [
            {"type": "low", "price": price, "index": idx} 
            for idx, price in swing_points['swing_lows'].dropna().iloc[-5:].items()
        ]
    ...
    def _update_trend_state(self, overall_trend: Dict[str, Any], df: pd.DataFrame) -> None:
        ...
        # Same trend continues
            ...
            (self.current_trend == 'up' and current_direction > 0) or
            (self.current_trend == 'down' and current_direction < 0)
        ):
            self.consecutive_signals += 1
    ...
    def _generate_signals(self, df: pd.DataFrame, overall_trend: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        signals = {
            "entry": None,  # 'long', 'short', or None
            "exit": None,   # 'exit' or None
            "confidence": 0.0,
            "sizing_factor": 1.0,
            "metadata": {}
        }
        ...
        if self.current_trend is not None and strong_trend:
            ...
            if self.current_trend == 'up' and self._check_uptrend_entry_conditions(df):
                ...
                signals["metadata"]["trend_duration"] = (current_time - self.trend_start_time).total_seconds() / 60 if hasattr(current_time, 'total_seconds') else 0
                ...
                self.logger.info(f"Generated LONG signal at {current_price} with confidence {confidence:.2f}")
                ...
                self.logger.info(f"Generated SHORT signal at {current_price} with confidence {confidence:.2f}")
    ...
    def _check_uptrend_entry_conditions(self, df: pd.DataFrame) -> bool:
        ...
        if self.current_market_regime in [REGIME_TYPES.STRONG_TREND, REGIME_TYPES.VOLATILE_TREND]:
            ...
            short_ma_pullback = (df['close'].iloc[-2] < df['short_ma'].iloc[-2] and 
                                close > df['short_ma'].iloc[-1])
            
            medium_ma_pullback = (close <= df['medium_ma'].iloc[-1] * 1.01 and 
                                close >= df['medium_ma'].iloc[-1] * 0.99)
            
            # MACD momentum confirmation
            macd_crossover = (df['macd'].iloc[-2] < df['macd_signal'].iloc[-2] and 
            ...
            medium_ma_support = (df['close'].iloc[-2] < df['medium_ma'].iloc[-2] and 
                                close > df['medium_ma'].iloc[-1])
            
            # Bounce off Keltner lower band
            keltner_bounce = (df['low'].iloc[-2] <= df['keltner_low'].iloc[-2] and 
                            close > df['keltner_low'].iloc[-1])
            ...
            strong_support_bounce = (df['low'].iloc[-3:].min() <= df['keltner_low'].iloc[-3:].min() and
                                    close > df['keltner_mid'].iloc[-1])
            
            # Strong MACD signal
            strong_macd = (df['macd_hist'].iloc[-1] > 0 and 
                        df['macd_hist'].iloc[-1] > df['macd_hist'].iloc[-2] * 1.5)
    ...
    def _check_downtrend_entry_conditions(self, df: pd.DataFrame) -> bool:
        ...
        if self.current_market_regime in [REGIME_TYPES.STRONG_TREND, REGIME_TYPES.VOLATILE_TREND]:
            ...
            short_ma_rally = (df['close'].iloc[-2] > df['short_ma'].iloc[-2] and 
                            close < df['short_ma'].iloc[-1])
            
            medium_ma_rally = (close >= df['medium_ma'].iloc[-1] * 0.99 and 
                            close <= df['medium_ma'].iloc[-1] * 1.01)
            ...
            medium_ma_resistance = (df['close'].iloc[-2] > df['medium_ma'].iloc[-2] and 
                                close < df['medium_ma'].iloc[-1])
            
            # Bounce off Keltner upper band
            keltner_bounce = (df['high'].iloc[-2] >= df['keltner_high'].iloc[-2] and 
                            close < df['keltner_high'].iloc[-1])
            ...
            # In highly volatile or choppy markets, be very selective
            # Consider only strong bounces with multiple confirmations
            strong_resistance_bounce = (df['high'].iloc[-3:].max() >= df['keltner_high'].iloc[-3:].max() and
                                      close < df['keltner_mid'].iloc[-1])
    ...
    def _check_exit_conditions(self, df: pd.DataFrame, overall_trend: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if trend_changed:
            return {
                ...
                "strength": 0.9  # High confidence exit
            }
        ...
        if profit_target_hit:
            return {
                ...
                "strength": 0.8  # High confidence exit
            }
        ...
        if stop_loss_hit:
            return {
                "reason": "stop_loss",
                "strength": 1.0  # Highest confidence exit - must exit
            }
        ...
        if time_stop_reached:
            ...
            in_profit = (self.current_trend == 'up' and current_price > avg_entry) or \
                       (self.current_trend == 'down' and current_price < avg_entry)
            ...
            if in_profit or trend_weakening:
                return {
                    "reason": "time_stop",
                    "strength": 0.7 if in_profit else 0.5
                }
            ...
            if potential_reversal:
                ...
                if in_profit:
                    return {
                        ...
                        "strength": 0.6  # Medium confidence exit
                    }
