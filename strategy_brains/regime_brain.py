...
from common.utils import calculate_sharpe_ratio, calculate_drawdown, get_utc_now
from common.async_utils import gather_with_concurrency
from common.constants import (
    TIMEFRAMES, MARKET_REGIMES, STRATEGY_TYPES, SIGNAL_TYPES,
    DEFAULT_REGIME_CONFIG, DEFAULT_RISK_PARAMETERS
)
...
class RegimeBrain(BaseBrain):
    ...
    def _initialize_regime_components(self):
        ...
        self.regime_classifier = ClassificationModel(
            ...
            model_path=self.config.get("regime_classifier_path", "models/regime_classifier")
        )
    ...
    async def detect_regime(self, market_data: pd.DataFrame) -> Tuple[MarketRegime, float]:
        ...
        try:
            ...
            if self.current_regime != MarketRegime.UNKNOWN and detected_regime != self.current_regime:
                ...
                self.logger.info(f"Regime transition detected: {self.current_regime} -> {detected_regime} "
                                f"with confidence {regime_confidence:.2f}")
            ...
            self.metrics_collector.record_gauge(
                f"regime_brain.regime_confidence.{self.exchange_id}.{self.asset_id}",
                regime_confidence
            )
            self.metrics_collector.record_gauge(
                f"regime_brain.regime_type.{self.exchange_id}.{self.asset_id}",
                detected_regime.value
            )
    ...
    async def _detect_regime_volume_profile(self, market_data: pd.DataFrame) -> Tuple[MarketRegime, float]:
        ...
        try:
            ...
            volume_data['volume_price_correlation'] = (
                volume_data['volume_momentum'] * 
                np.sign(volume_data['price_change'])
            ...
            if (recent_data['volume_momentum'].mean() > 1.2 and 
                recent_data['price_change'].mean() > -0.001 and
                recent_data['close'].iloc[-1] < recent_data['vwap'].iloc[-1]):
    ...
    async def _detect_regime_support_resistance(self, market_data: pd.DataFrame) -> Tuple[MarketRegime, float]:
        ...
        try:
            ...
            resistance_fails = sum(1 for i in range(-10, 0) if i < -1 and 
                                market_data['high'].iloc[i] > nearest_resistance > market_data['close'].iloc[i])
            if resistance_fails >= 2:
                return MarketRegime.WEAK_UPTREND, 0.6
            
            # Multiple failed attempts to break support
            support_fails = sum(1 for i in range(-10, 0) if i < -1 and 
                             market_data['low'].iloc[i] < nearest_support < market_data['close'].iloc[i])
            if support_fails >= 2:
    ...
    def _calculate_bollinger_bandwidth(self, df: pd.DataFrame, period: int = 20, 
    ...
    async def analyze(self, market_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            combined_signals = self._combine_signals(
                ...
                strategy_weights["primary"], strategy_weights["secondary"]
            )
            ...
            self.metrics_collector.record_timing(
                f"regime_brain.analysis_time.{self.exchange_id}.{self.asset_id}",
    ...
    def _adjust_parameters_by_confidence(self, parameters: Dict[str, float], 
    ...
    async def _apply_strategy(self, strategy_type: str, market_data: pd.DataFrame, 
    ...
    async def _apply_trend_following(self, market_data: pd.DataFrame, 
                                 parameters: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _apply_mean_reversion(self, market_data: pd.DataFrame, 
                               parameters: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _apply_breakout(self, market_data: pd.DataFrame, 
                          parameters: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _apply_momentum(self, market_data: pd.DataFrame, 
                         parameters: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _apply_volatility(self, market_data: pd.DataFrame, 
                           parameters: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _apply_support_resistance(self, market_data: pd.DataFrame,
                                    parameters: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def _apply_neutral(self, market_data: pd.DataFrame,
                        parameters: Dict[str, float]) -> Dict[str, Any]:
    ...
    def _combine_signals(self, primary_signals: Dict[str, Any], secondary_signals: Dict[str, Any],
                       primary_weight: float, secondary_weight: float) -> Dict[str, Any]:
        ...
        if primary_signals["signal"] == secondary_signals["signal"]:
            combined["confidence"] = min(
                primary_signals["confidence"] * primary_weight + 
                secondary_signals["confidence"] * secondary_weight,
                0.95
            )
        ...
        combined["indicators"] = {
            "primary": primary_signals.get("indicators", {}),
            "secondary": secondary_signals.get("indicators", {})
        }
        
        return combined
    
    async def _generate_regime_insights(self, regime: MarketRegime, 
    ...
    def _get_regime_description(self, regime: MarketRegime) -> str:
        ...
        descriptions = {
            MarketRegime.STRONG_UPTREND: "A powerful uptrend with consistent higher highs and higher lows. Strong buying pressure with limited pullbacks. Momentum indicators are bullish with good volume confirmation.",
            MarketRegime.WEAK_UPTREND: "An uptrend that is losing momentum or facing resistance. Still making higher highs and higher lows, but with decreasing strength. May be transitioning to a ranging or distribution phase.",
            MarketRegime.RANGING: "Price moving sideways within a defined range. Multiple tests of support and resistance with neither buyers nor sellers gaining control. Often a period of consolidation before the next directional move.",
            MarketRegime.WEAK_DOWNTREND: "A downtrend that is losing momentum or finding support. Still making lower highs and lower lows, but with decreasing strength. May be transitioning to a ranging or accumulation phase.",
            MarketRegime.STRONG_DOWNTREND: "A powerful downtrend with consistent lower highs and lower lows. Strong selling pressure with limited rallies. Momentum indicators are bearish with good volume confirmation.",
            MarketRegime.HIGH_VOLATILITY: "Elevated price fluctuations with larger than normal candles and wide ranges. Often seen during market uncertainty, news events, or during trend reversals. Risk is heightened in both directions.",
            MarketRegime.LOW_VOLATILITY: "Compressed price action with smaller than normal candles and narrow ranges. Often precedes a volatility expansion and can signal a coming powerful move. Patience is required during this phase.",
            MarketRegime.TRANSITIONING: "Market in flux between regimes. Old patterns breaking down while new patterns have not yet established. Often marked by increased volatility and conflicting signals.",
            MarketRegime.ACCUMULATION: "Sideways price action near support levels with decreasing selling pressure. Early smart money beginning to accumulate positions before a potential uptrend. Often characterized by decreasing volume but absorption of selling.",
            MarketRegime.DISTRIBUTION: "Sideways price action near resistance levels with decreasing buying pressure. Smart money beginning to distribute (sell) positions before a potential downtrend. Often characterized by decreasing volume but absorption of buying."
        }
    ...
    async def _detect_regime_inefficiencies(self, regime: MarketRegime, 
    ...
    def _get_historical_performance(self, regime: MarketRegime) -> Dict[str, Any]:
        ...
        return {
            ...
            "regime_effectiveness": "Unknown - Insufficient data"
        }
    
    async def _calculate_transition_probability(self, current_regime: MarketRegime,
    ...
    async def update_performance(self, trade_result: Dict[str, Any]) -> None:
        ...
        if "regime" in trade_result and "success" in trade_result and "pnl_percent" in trade_result:
            ...
            if regime and regime in self.regime_performance:
                ...
                perf = self.regime_performance[regime]
                win_rate = perf["success"] / perf["count"] if perf["count"] > 0 else 0
                self.logger.info(f"Updated {regime.name} performance: {perf['count']} trades, "
                               f"{win_rate:.2f} win rate, {perf['pnl']:.2f}% cumulative PnL")
    ...
    def from_dict(cls, data: Dict[str, Any], 
                 ...
                 market_data_repository=None) -> 'RegimeBrain':
