...
from common.utils import timeit, find_peaks, circular_buffer
from data_storage.market_data import MarketDataStorage
from feature_service.features.market_structure import (
    MarketStructureFeatures, SwingPointFeatures, ValueAreaFeatures
)
...
class MarketStructureBrain(StrategyBrain):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            candles = await self.market_data.get_candles(
                self.asset_id, 
                self.timeframe, 
                limit=max(200, self.params["phase_detection_periods"] * 2)
            )
    ...
    async def generate_signal(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"Generated signal: {signal['id']} Type: {signal['type']} Direction: {'BUY' if signal['direction'] > 0 else 'SELL'} Strength: {signal['strength']:.2f}")
    ...
    async def update_parameters(self, performance_metrics: Dict[str, float]) -> None:
        ...
        if adjustment != 0:
            ...
            self.logger.info(f"Parameters adjusted: swing_threshold={self.params['swing_strength_threshold']:.2f}, " +
    ...
    async def _process_historical_candles(self) -> None:
        ...
        swing_points = await self.swing_point_features.identify_swing_points(
            ...
            strength_threshold=self.params["swing_strength_threshold"]
        )
        ...
        value_areas = await self.value_area_features.calculate_value_areas(
            self.candle_history[-self.params["value_area_lookback"]:],
            value_area_percentage=self.params["value_area_percentage"]
        )
    ...
    async def _update_market_structure(self, candle: Dict[str, Any]) -> None:
        ...
        if self.data_points_processed % 10 == 0:
            self.current_market_phase = await self._detect_market_phase()
            self.market_phase_history.append({
                'timestamp': candle['timestamp'],
        ...
        if self.data_points_processed % 5 == 0:
            ...
            self.value_areas = {
                ts: va for ts, va in self.value_areas.items() 
                if ts > old_threshold
            }
        ...
        if swing_type == 'low':
            ...
            for i, level in enumerate(self.support_levels):
                ...
                if abs(level['price'] - price) / price < self.params["sr_proximity_ratio"]:
                    ...
                    new_price = (level['price'] * level['strength'] * (1-weight) + 
                                price * strength * weight) / (level['strength'] * (1-weight) + strength * weight)
    ...
    async def _cleanup_sr_levels(self) -> None:
        ...
        self.support_levels = [
            ...
            if (level['touch_count'] >= self.params["sr_touch_threshold"] or 
                (now - level['last_touch'] < max_age_ms and level['strength'] >= self.params["sr_significance_threshold"]))
        ]
    ...
    async def _detect_market_phase(self) -> str:
        ...
        if (abs(second_half_change) < 0.02 and 
            self.current_trend in ['neutral', 'downtrend'] and
            second_half_volatility < first_half_volatility * (1 - self.params["phase_volatility_threshold"]) and
            volume_change > self.params["phase_volume_threshold"]):
            return 'accumulation'
        
        # Markup: prices rising, increased volume
        elif (second_half_change > 0.03 and 
              second_half_change > first_half_change and
              volume_change > 0):
            return 'markup'
        
        # Distribution: sideways after uptrend, decreasing volatility, decreasing volume
        elif (abs(second_half_change) < 0.02 and 
              self.current_trend in ['neutral', 'uptrend'] and
              second_half_volatility < first_half_volatility * (1 - self.params["phase_volatility_threshold"]) and
              volume_change < -self.params["phase_volume_threshold"]):
            return 'distribution'
