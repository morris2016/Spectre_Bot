...
class TradingEnvironment:
    ...
    def _get_observation(self):
        """Return a padded window of feature data."""
        start = max(0, self.current_step - self.window_size)
        window = self.data.iloc[start:self.current_step][self.feature_columns].values
        if len(window) < self.window_size:
            pad_rows = self.window_size - len(window)
            pad = np.zeros((pad_rows, window.shape[1]))
            window = np.vstack([pad, window])

        if self.use_position_info:
            pos_info = np.zeros((self.window_size, 2))
            if self.max_position:
                pos_info[:, 0] = self.position / self.max_position
            if self.entry_price:
                current_price = self.data.iloc[self.current_step - 1]["close"]
                pos_info[:, 1] = (current_price - self.entry_price) / current_price
            window = np.hstack((window, pos_info))
