...
from common.constants import (
    ...
    DEFAULT_ADAPTIVE_CONFIG, DEFAULT_RISK_PARAMETERS
)
from common.logger import get_logger
from common.metrics import MetricsCollector
from common.exceptions import StrategyExecutionError, AdaptationError
...
class AdaptiveBrain(BaseBrain):
    ...
    def _initialize_strategy_variants(self):
        ...
        self.logger.info(f"Initialized {len(self.strategy_variants)} strategy variants, "
                       f"{len(self.active_variants)} active variants")
    ...
    def _update_strategy_weights(self):
        ...
        # Get active variants
        active_variants = [self.strategy_variants[v_id] for v_id in self.active_variants 
                         if v_id in self.strategy_variants]
        ...
        if total_fitness <= 0:
            ...
            self.strategy_weights = {
                v.variant_id: v.fitness_score / total_fitness for v in active_variants
            }
            
            # Log top strategies
            top_strategies = sorted(
                [(v.variant_id, self.strategy_weights[v.variant_id]) for v in active_variants],
                key=lambda x: x[1],
                reverse=True
            )
    ...
    def _select_primary_secondary_variants(self):
        ...
        if len(sorted_variants) >= 1:
            ...
            if self.primary_variant_id in self.strategy_variants:
                ...
                self.logger.info(f"Primary variant: {primary.variant_id} ({primary.strategy_type}) "
        ...
        if len(sorted_variants) >= 2:
            ...
            if self.secondary_variant_id in self.strategy_variants:
                ...
                self.logger.info(f"Secondary variant: {secondary.variant_id} ({secondary.strategy_type}) "
    ...
    async def analyze(self, market_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy_info = {
                ...
                "primary_type": self.strategy_variants[self.primary_variant_id].strategy_type if self.primary_variant_id in self.strategy_variants else None,
                "primary_weight": self.strategy_weights.get(self.primary_variant_id, 0),
                "secondary_variant": self.secondary_variant_id,
                "secondary_type": self.strategy_variants[self.secondary_variant_id].strategy_type if self.secondary_variant_id in self.strategy_variants else None,
            ...
            self.metrics_collector.record_timing(
                f"adaptive_brain.analysis_time.{self.exchange_id}.{self.asset_id}",
    ...
    async def _apply_strategy_variant(self, variant: StrategyVariant, 
