...
from feature_service.features.volatility import (
    volatility_smile, volatility_regime_classification, 
    calculate_historical_volatility, implied_volatility_percentile
)
...
class VolatilityBrain(StrategyBrain):
    ...
    async def analyze(self, market_data: pd.DataFrame, context: Dict[str, Any] = None) -> List[SignalEvent]:
        ...
        try:
            ...
            regime_signals = self._generate_regime_based_signals(
                market_data, 
                volatility_metrics, 
                volatility_regime
            )
            ...
            for signal in regime_signals:
                complete_signal = self._generate_complete_signal(
                    signal, 
                    market_data, 
                    volatility_metrics['atr_latest']
                )
                if complete_signal:
    ...
    def _generate_regime_based_signals(
        self, 
        data: pd.DataFrame, 
        volatility_metrics: Dict[str, Any], 
        ...
        if volatility_regime == 'squeeze' or 'low_increasing' in volatility_regime:
            ...
            if (recent_data['close'].iloc[-1] > upper_band or 
                recent_data['high'].iloc[-1] > upper_band):
                
                # Verify momentum with increasing volume
                if (recent_data['volume'].iloc[-1] > data['volume'].iloc[-5:-1].mean() * 1.2 and
            ...
            elif (recent_data['close'].iloc[-1] < lower_band or 
                  recent_data['low'].iloc[-1] < lower_band):
        ...
        if volatility_regime in ['increasing', 'decreasing']:
            ...
            trend_up = (latest_close > ema20.iloc[-1] > ema50.iloc[-1] and
                       ema20.iloc[-1] > ema20.iloc[-5])
            
            trend_down = (latest_close < ema20.iloc[-1] < ema50.iloc[-1] and
                         ema20.iloc[-1] < ema20.iloc[-5])
            ...
            if volatility_regime == 'increasing':
                if trend_up:
                    ...
                    if (data['low'].iloc[-1] <= ema20.iloc[-1] <= data['high'].iloc[-1] or
                        abs(latest_close - ema20.iloc[-1]) < latest_atr * 0.5):
    ...
    def _generate_complete_signal(
        ...
        if signal_type == 'squeeze_breakout':
            ...
            take_profit = entry_price + (risk * self.config.risk_reward_min) if direction == 'up' else entry_price - (risk * self.config.risk_reward_min)
    ...
    def _calculate_mean_reversion_confidence(
        ...
        if direction == 'up':
            reversal_candle = (
                data['close'].iloc[-1] > data['open'].iloc[-1] and  # Bullish candle
                data['low'].iloc[-1] < data['low'].iloc[-2]  # New low
            )
        else:
            reversal_candle = (
                data['close'].iloc[-1] < data['open'].iloc[-1] and  # Bearish candle
                data['high'].iloc[-1] > data['high'].iloc[-2]  # New high
            )
    ...
    def _calculate_trend_confidence(
        ...
        if direction == 'up':
            good_entry = (
                data['close'].iloc[-1] > ema20.iloc[-1] and
                data['low'].iloc[-1] <= ema20.iloc[-1] * 1.01  # Close to EMA
            )
        else:
            good_entry = (
                data['close'].iloc[-1] < ema20.iloc[-1] and
                data['high'].iloc[-1] >= ema20.iloc[-1] * 0.99  # Close to EMA
            )
    ...
    def _update_adaptive_parameters(self, asset: str) -> None:
        ...
        self.asset_volatility_characteristics[asset] = {
    ...
    def _adjust_based_on_effectiveness(self, asset: str) -> None:
        ...
        if asset in self.asset_mean_reversion_effectiveness:
            ...
            if mr_effectiveness.get('win_rate', 0) > 0.7:
                ...
                self.config.asset_specific_config[asset]['mean_reversion_overextension'] = max(
                    1.8, 
                    self.config.mean_reversion_overextension * 0.9
                )
                ...
                self.config.asset_specific_config[asset]['mean_reversion_overextension'] = min(
                    4.0, 
                    self.config.mean_reversion_overextension * 1.2
                )
        ...
        if asset in self.asset_breakout_effectiveness:
            ...
            if bo_effectiveness.get('win_rate', 0) > 0.7:
                ...
                self.config.asset_specific_config[asset]['volatility_breakout_multiplier'] = max(
                    1.0, 
                    self.config.volatility_breakout_multiplier * 0.9
                )
                ...
                self.config.asset_specific_config[asset]['volatility_breakout_multiplier'] = min(
                    2.5, 
                    self.config.volatility_breakout_multiplier * 1.2
                )
    ...
    async def on_signal_result(self, signal: SignalEvent, result: Dict[str, Any]) -> None:
        ...
        if 'mean_reversion' in strategy_type:
            ...
            if asset not in self.asset_mean_reversion_effectiveness:
                self.asset_mean_reversion_effectiveness[asset] = {
                    'wins': 0,
                    'total': 0,
                    'profit_sum': 0,
                    'win_rate': 0
            ...
            if asset not in self.asset_breakout_effectiveness:
                self.asset_breakout_effectiveness[asset] = {
                    'wins': 0,
                    'total': 0,
                    'profit_sum': 0,
                    'win_rate': 0
        ...
        # Log key information
        logger.info(f"Volatility Brain signal result for {asset} ({strategy_type}): Win={is_win}, Profit={profit_pct:.2f}%, Regime={volatility_regime}")
