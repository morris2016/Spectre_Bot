...
# Internal imports
from common.utils import ParallelProcessor, resample_data, calculate_zscore
from common.constants import (
    DEFAULT_LOOKBACK_PERIODS, DEFAULT_CONFIDENCE_LEVELS, DEFAULT_PAIR_CORR_THRESHOLD,
    DEFAULT_COINTEGRATION_PVALUE_THRESHOLD, DEFAULT_ZSCORE_ENTRY_THRESHOLD, 
    DEFAULT_ZSCORE_EXIT_THRESHOLD, DEFAULT_HALF_LIFE_MULTIPLIER,
    MARKET_REGIMES
)
...
class StatisticalBrain(BaseBrain):
    ...
    def __init__(
        ...
        self.cointegration_pvalue_threshold = self.parameters.get("cointegration_pvalue_threshold", 
    ...
    async def _fetch_pair_data(self, pair: str) -> pd.DataFrame:
        ...
        if self.market_data_repo:
            try:
                return await self.market_data_repo.get_historical_data(
                    asset_id=pair,
    ...
    async def _find_cointegrated_pairs(
        self, data: pd.DataFrame, related_data: Dict[str, pd.DataFrame]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_success_probability(self, metrics: Dict[str, Any], signal: int) -> float:
        ...
        for i in range(1, len(zscore_history)):
            if (zscore_history[i-1] < 0 and zscore_history[i] >= 0) or \
               (zscore_history[i-1] > 0 and zscore_history[i] <= 0):
                crossover_count += 1
    ...
    async def _generate_signals(
        self, opportunities: List[Dict[str, Any]], position_sizes: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        ...
        if signals:
            overall_confidence = sum(s["confidence"] * s["position_size"] for s in signals) / \
                               sum(s["position_size"] for s in signals) if sum(s["position_size"] for s in signals) > 0 else 0
        else:
    ...
    async def _process_learning(self, performance_data: Dict[str, Any]):
        ...
        try:
            ...
            if successful_thresholds:
                ...
                logger.info(f"Statistical Brain learned new thresholds: entry={self.zscore_entry_threshold:.2f}, exit={self.zscore_exit_threshold:.2f}")
